...
Widget footer(Context context) {
  return Footer(
    trailing: Container(height: 25, child: Text('Page ${context.pageNumber}')),
  );
}

Widget header(Context context) {
  return Container(height: 20, child: Text('Test document'));
}
...
List<Widget> contentWithPageBreak(double? freeSpace) {
  return [
    ...
    Container(color: const PdfColor(0.6, 0.6, 0.6), height: 40, width: 100),
    Container(color: const PdfColor(0.5, 0.5, 0.5), height: 20, width: 100),
...
void main() {
  ...
  const pageFormatWithMargins = PdfPageFormat(
    200.0,
    200.0,
    marginTop: 10,
    marginBottom: 20,
